    ...
    <div class="text-center">
      ...
      <v-row>
        <v-col v-for="(item, index) of listaC" :key="index">
          <iframe width="360" height="215" :src="'https://www.youtube.com/embed/'+item.cod"
...
export default {
   ...
   */
  async beforeCreate () {
      let datos = new FormData(); datos.append('func', 17);
      await this.axios.post(this.axios.default.baseURL, datos)
      .then (res => {
  ...
  methods : {
    infoCanciones (min, max) {
      let datos = new FormData(); datos.append('func', 18);
      datos.append('min', min); datos.append('max', max);
